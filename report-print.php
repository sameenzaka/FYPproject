...
$area=$_POST["area"];
$price=$_POST["price"];
$agentcom=$_POST["agent_commission"];
$buyerpaid=$_POST["buyer_paid"];
$paystatus=$_POST["payment_type"];

if($action == "add"){
$sql = "INSERT INTO invoice (seller_id , seller_email , seller_phone , date , buyer_id , buyer_address , buyer_phone , city , property_type , area , price , agent_commission , buyer_paid , payment_status) VALUES('$sellerid' , '$selleremail' , '$sellerphone' , '$date' , '$buyerid' , '$buyeraddress' , '$buyerphone' , '$city' , '$propertytype' , '$area' , '$price' , '$agentcom' , '$buyerpaid' , '$paystatus')";
$result = mysqli_query($conn,$sql) or die(mysqli_error($conn));

if($result){ 
$invoices_id = mysqli_insert_id($conn);
?>
...
?>	
	...
	<?php 
	$sql = "SELECT * FROM invoice c inner join seller s on c.seller_id = s.seller_id WHERE invoice_id="."$invoices_id";
	$result = mysqli_query($conn,$sql);
	if(mysqli_num_rows($result)>0){
	while($row=mysqli_fetch_assoc($result)){
	?>
...
<?php 
$sql = "SELECT * FROM invoice c inner join buyer s on c.buyer_id = s.buyer_id WHERE invoice_id="."$invoices_id";
$result = mysqli_query($conn,$sql);
