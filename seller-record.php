...
<!--//nav-item-->
	...
	<!-- <div class="container" style="max-width: 12s00px; width: 100% ; padding: 25px 30px ; border-radius: 25px ; background: linear-gradient(135deg, #dcdcdc, #f8b425);"> -->
		<div class="heading" style="font-size: 25 ; font-weight: 500 ; position: relative ; text-align: center ; padding: 0 0 20px 0;">Property Records</div>
			<table class="table">
			...
			$sql = "SELECT * FROM addhouse where agent_area=(SELECT agent_area FROM agent where id ='{$agent_id}')";
			$result = mysqli_query($conn,$sql);
			if(mysqli_num_rows($result)>0){
			while($row=mysqli_fetch_assoc($result)){
			?>
			<tbody>
				<tr>
					...
					<td>
						...
						<a class="btn btn-primary btn-sm" href="add-house.php?houid=<?php echo $row["house_id"];?>&action=update">Edit</a>
						<a class="btn btn-danger btn-sm" href="seller-record.php?houid=<?php echo $row["house_id"];?>&action=delete">Delete</a>
